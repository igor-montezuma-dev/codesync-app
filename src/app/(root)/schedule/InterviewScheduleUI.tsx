...
function InterviewScheduleUI() {
  ...
  const interviews = useQuery(api.interviews.getAllInterwiews) ?? [];
  ...
  const scheduleMeeting = async () => {
    ...
    if (!formData.candidateId || formData.interviewerIds.length === 0) {
      toast.error(
        "Por favor, selecione um candidato e ao menos um entrevistador"
      );
      return;
    }
    setIsCreating(true);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Agende e administre suas entrevistas
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button size="lg">Agendar Entrevista</Button>
          </DialogTrigger>
          ...
          <DialogContent className="sm:max-w-[500px] h-[calc(100vh-200px)] overflow-auto">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Entrevistadores</label>
                ...
                {availableInterviewers.length > 0 && (
                  <Select onValueChange={addInterviewer}>
                    <SelectTrigger>
                      <SelectValue placeholder="Adicione um entrevistador" />
                    </SelectTrigger>
                    <SelectContent>
